...
> 本教程适用于 Windows 版 WordReader。建议第一次使用时按顺序完成“启动 → 登录 → 书架或发现 → 目录 → 阅读”。
...
1. 双击项目根目录中的 `启动阅读器.cmd`。
2. 在顶部蓝色区域输入 `/登录`，完成起点官方账号登录。
3. 输入 `/书架`，等待书库显示为带序号的文字列表。
4. 输入书籍序号，再输入章节序号；也可以输入 `/继续`。
5. 章节完成 OCR 后，直接敲击普通按键显示正文。
6. 阅读中输入 `/下一章`、`/上一章`、`/目录` 或 `/返回` 进行导航。

如果不想从书架开始，可以输入 `/搜索 关键词`、`/分类` 或 `/排行` 找书。
...
不要直接在压缩包预览窗口中运行程序，否则 OCR 模型、WebView2 依赖或登录目录可能无法正常访问。
...
3. **Word 功能区**：保留字体、段落、样式、编辑等视觉结构，并将部分按钮替换为阅读功能。
4. **A4 文档区**：书架、目录、详情和 OCR 正文都会以文档文字形式显示。
...
![样式菜单与命令操作](../assets/screenshots/style-gallery.png)
...
1. 输入 `/登录`。
2. 程序会临时显示起点官方登录页面。
3. 使用账号密码、扫码或微信等官方登录方式完成认证。
4. 第三方验证弹窗会在共享 WebView2 登录配置的内置窗口中打开。
5. 程序确认官方登录状态后，会返回 Word 风格文档页面。

### 登录注意事项

- 看到起点网页能够进入书架，才代表账号登录流程真正完成。
- 如果之前退出过账号或登录状态过期，需要重新执行 `/登录`。
...
分类功能会依次读取起点官方页面提供的主分类、子分类、连载状态、字数、标签和排序条件。输入对应序号逐级选择。
